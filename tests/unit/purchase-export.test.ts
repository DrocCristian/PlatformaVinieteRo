...
test('private export includes new drafts and fleet tables, uses authenticated RLS client and no-store',async()=>{
 ...
 const db={auth:{getUser:async()=>({data:{user:{id:'user-a',email:'a@example.test'}},error:null})},from:(table:string)=>{
  ...
  const builder={select:()=>builder,eq:(key:string,value:unknown)=>{read.filters.push([key,value]);return builder;},order:()=>builder,range:()=>builder,abortSignal:async()=>({data:[],error:null})};return builder;
 }};
 const out={exports:{} as {GET:(request:Request)=>Promise<Response>}};
 ...
 runInNewContext(code,{exports:out.exports,Response,require:(name:string)=>{if(name==='../../../../packages/domain/personal-export')return exportModule;assert.equal(name,'../../../../lib/supabase/server');return {supabaseServer:async()=>db};}});
 const result=await out.exports.GET(new Request('https://vignexo.test/cont/date/export'));assert.equal(result.status,200);assert.equal(result.headers.get('Cache-Control'),'private, no-store');
