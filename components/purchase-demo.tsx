...
export default function PurchaseDemo({ saved, onPrepare, locale = 'ro', messages = {} }: {
    locale?: Locale;
    messages?: Messages;
    ...
    const error = (id: string) => errors[id] ? <span className="purchase-error" id={id + '-error'}>{t(errors[id])}</span> : null;
    function next() {
  ...
  <header className="purchase-header"><Link href="/" className="purchase-logo">vignexo<span>↗</span></Link><span>{t("Vinietele tale. Mai simplu.")}</span><Link href="/cont">{t("Contul meu")}</Link></header>
  <div className="purchase-notice">{t("PREVIZUALIZARE \u00B7 Nu se emit viniete \u0219i nu se \u00EEncaseaz\u0103 pl\u0103\u021Bi.")}</div>
  <main className="purchase-main">
   <div className="purchase-heading"><p className="purchase-eyebrow">{t("PREG\u0102TIT PENTRU URM\u0102TORUL DRUM")}</p><h1>{t("Alege perioada.")}<br /><span>{t("Noi adun\u0103m detaliile.")}</span></h1><p>{t("Un vehicul, mai multe \u021B\u0103ri, un singur rezumat.")}</p></div>
   ...
   <div className="purchase-grid"><form ref={form} noValidate onSubmit={e => { e.preventDefault(); next(); }} className="purchase-card">
    ...
    {step === 1 && <>
     ...
     <div className="purchase-fields"><label htmlFor="registration">{t("\u021Aara de \u00EEnmatriculare")}<select {...attrs('registration')} value={registration} onChange={e => setRegistration(e.target.value)}>{[...countries, { code: 'DE', name: t("Germania") }, { code: 'IT', name: t("Italia") }, { code: 'FR', name: t("Fran\u021Ba") }, { code: 'PL', name: 'Polonia' }, { code: 'GR', name: 'Grecia' }, { code: 'UA', name: 'Ucraina' }, { code: 'GB', name: 'Regatul Unit' }].map(c => <option key={c.code} value={c.code}>{countryName(c.code)}</option>)}</select>{error('registration')}</label>
     <label htmlFor="plate">{t("Num\u0103r de \u00EEnmatriculare")}<input {...attrs('plate')} value={plate} maxLength={20} autoComplete="off" placeholder={t("Ex. TM 12 ABC")} onChange={e => setPlate(e.target.value)}/>{error('plate')}</label></div>
     <label htmlFor="vin">{t("VIN / seria de \u0219asiu")}<span className="purchase-optional">{t("\u2014 dac\u0103 este cerut\u0103 de \u021Bara aleas\u0103")}</span><input {...attrs('vin')} value={vin} maxLength={32} autoComplete="off" placeholder={t("Din c\u00E2mpul E al talonului")} onChange={e => setVin(e.target.value.toUpperCase())}/>{error('vin')}</label>
    ...
    {step === 2 && <>
     ...
     {selections.map(s => {
                ...
                return <section key={s.country} className="purchase-country-card"><header><h3><Flag code={s.country}/>{countryName(s.country)}</h3><button type="button" aria-label={'Elimină ' + s.country} onClick={() => toggle(s.country)}><X size={18}/></button></header>
     ...
     <details className="purchase-technical"><summary>{t("Date pentru \u00EEncadrare din talon")}</summary><p>{t("Necesar \u00EEnainte de oferta final\u0103: categoria, masele \u0219i, dup\u0103 \u021Bar\u0103, axele, emisiile \u0219i configura\u021Bia ansamblului. Datele de aici nu produc un tarif.")}</p><div className="purchase-fields">{[['category', t("Categoria J")], ['f1', t("Masa F.1 (kg)")], ['f2', t("Masa F.2 (kg)")], ['f3', t("Masa ansamblului F.3 (kg)")], ['axles', t("Axe vehicul")], ['seats', t("Locuri S.1")], ['euro', t("Norma EURO")], ['co2', t("Clasa CO\u2082")], ...(trailer ? [['trailer-f1', t("Remorc\u0103 F.1 (kg)")], ['trailer-f2', t("Remorc\u0103 F.2 (kg)")], ['trailer-axles', t("Axe remorc\u0103")]] : [])].map(([id, label]) => <label key={id} htmlFor={'technical-' + id}>{t(label)}<input readOnly={!!saved} id={'technical-' + id} value={technicalProfile[id] ?? ''} maxLength={20} onChange={e => setTechnicalProfile({ ...technicalProfile, [id]: e.target.value })}/></label>)}</div></details>
   ...
   </form><aside className="purchase-card purchase-summary"><span className="purchase-eyebrow">{t("C\u0102L\u0102TORIA TA")}</span><h2>{t("Totul \u00EEntr-un loc")}</h2><div className="purchase-summary-vehicle"><CarFront size={25}/><span>{plate ? normalizePlate(plate) : t("Vehiculul t\u0103u")}<small>{types.find(t => t.id === vehicle)?.title}</small></span></div>{selections.length === 0 ? <p className="purchase-empty">{t("\u021A\u0103rile \u0219i perioadele alese vor ap\u0103rea aici.")}</p> : selections.map(s => <div className="purchase-summary-line" key={s.country}><Flag code={s.country}/><div><strong>{countries.find(c => c.code === s.country)!.name}</strong><small>{duration(s.duration, availableDurations(s.country, vehicle).find(o => o.id === s.duration)?.label ?? t('De selectat / verificat'))}{s.duration === 'annual' ? ' ' + s.year : ''}</small>{s.start && <small><CalendarDays size={12}/> {formatTravelDate(s.start)}</small>}</div><span>—</span></div>)}<div className="purchase-total"><span>{t("Total de plat\u0103")}</span><strong>{t("\u00CEn a\u0219teptarea ofertei")}</strong></div><p>{t("Taxa emitentului, serviciul Vignexo \u0219i totalul vor fi afi\u0219ate separat \u00EEnainte de plat\u0103.")}</p><div className="purchase-trust"><ShieldCheck size={20}/><span>{t("O sum\u0103 final\u0103 numai dup\u0103 verificarea tuturor produselor.")}</span></div><Link href="/flote/demo">{t("Ai mai multe ma\u0219ini? Vezi Vignexo Business \u2192")}</Link></aside></div>
   <footer className="purchase-footer">{t("Vinieta acoper\u0103 re\u021Beaua \u0219i categoria produsului ales. Podurile, tunelurile, taxele kilometrice \u0219i restric\u021Biile de circula\u021Bie se verific\u0103 separat.")}</footer>
  </main>
 </div>;
}
