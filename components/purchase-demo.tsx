...
export default function PurchaseDemo({ saved, onPrepare, embedded = false, locale = 'ro', messages = {} }: {
  ...
  {!embedded && <PurchaseHeader locale={locale} messages={messages} demo/>}
  ...
  <Content className="purchase-main">
   <div className="purchase-heading">
    <p className="purchase-eyebrow">{t("Viniete digitale pentru Europa")}</p>
    <h1>{locale === 'ro' ? ['Mai puține griji. Mai mult drum.', 'Alege țările. Noi adunăm detaliile.', 'Un ultim pas. Verifică detaliile.'][step-1] : t('Viniete digitale pentru Europa')}</h1>
    <p>{t("Un vehicul, mai multe țări, un singur rezumat.")}</p>
   </div>
   <ol className="purchase-steps" aria-label={t("Pașii comenzii")}>{[t("Vehiculul tău"), t("Țări și durate"), locale==='ro'?'Verificare':t("Verificare și plată")].map((label, i) => <li key={label} className={step>i+1?'is-complete':undefined} aria-current={step === i + 1 ? 'step' : undefined}><span>{step > i + 1 ? <Check size={17}/> : i + 1}</span>{label}</li>)}</ol>
   <div className="purchase-grid"><form ref={form} noValidate onSubmit={e => { e.preventDefault(); next(); }} className="purchase-card purchase-form">
    ...
    {step === 2 && <>
     ...
     {simpleCar && <PassengerCarDetails mode="declaration" answers={carAnswers} countries={[]} errors={errors} onChange={answers=>{setCarAnswers(answers);setErrors({});setConfirmed(false);}}/>}
     {selections.map(s => {
      ...
      {simpleCar && ['HU','SI','CZ'].includes(s.country) && <PassengerCarDetails mode="country" answers={carAnswers} countries={[s.country]} errors={errors} onChange={answers=>{setCarAnswers(answers);setErrors({});setConfirmed(false);}}/>}
    ...
    {step === 3 && <>
     ...
     <button type="button" className="purchase-back purchase-edit" onClick={()=>go(1)}>{t('Editează datele vehiculului')}</button>
     ...
     {Object.values(technicalProfile).some(Boolean) && <dl className="purchase-technical-review">{[['category',t('Categoria J')],['f1',t('Masa F.1 (kg)')],['f2',t('Masa F.2 (kg)')],['seats',t('Locuri S.1')],['axles',t('Axe vehicul')],['euro',t('Norma EURO')]].filter(([key])=>technicalProfile[key]).map(([key,label])=><div key={key}><dt>{label}</dt><dd>{technicalProfile[key]}</dd></div>)}</dl>}
