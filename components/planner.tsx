    ...
    <div className="planner-grid">
      <div className="selection">
        {step===1?<><h2 ref={titleRef} tabIndex={-1} className="sr-only">{t("Pe unde călătorești?")}</h2><EuropeMap messages={messages} selected={selected} onToggle={onToggle}/>
          ...
          {step===2?<form id="vehicle-form" onSubmit={event=>{event.preventDefault();setPeriodError('');if(!isPreviewPlateValid(plate)){setError(t("Introdu între 2 și 12 litere sau cifre pentru această previzualizare."));return;}for(const code of selected){const problem=validateTravelPeriod(periods[code]??{entry:'',exit:''},localToday());if(problem){setError('');setPeriodError(t(countries.find(c=>c.code===code)!.name)+': '+t(problem));return;}}goTo(3);}}>
            ...
            <label htmlFor="category">{t("Vehicul pentru această previzualizare")}</label><select id="category"><option>{t("Autoturism — categorie de confirmat")}</option></select><p className="info-note"><Info size={16}/>{t("Eligibilitatea se verifică separat pentru fiecare produs.")}</p>
      ...
      <aside className="summary"><div className="summary-heading"><h3>{t("Țările selectate")}</h3><button onClick={()=>{goTo(1);document.getElementById('destinatii')?.scrollIntoView({block:'nearest'});}}>{t("Editează")}</button></div>
        ...
        <div className="vehicle-summary"><div className="summary-heading"><h3>{t("Vehiculul meu")}</h3><button disabled={!selected.length} onClick={()=>goTo(2)}>{plate?t("Editează"):t("Adaugă")}</button></div><div className="vehicle-row"><CarFront size={27}/><div><strong>{t("Autoturism")}</strong><small>{plate?normalizePlate(plate):t("Completează datele vehiculului")}</small></div><ChevronRight size={15}/></div></div>
