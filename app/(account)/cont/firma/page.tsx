...
export default async function CompanyPage({searchParams}:{searchParams:Promise<{company?:string}>}){
 ...
 db.from('fleet_assets').select('id,company_id,plate,registration_country,label,kind,f1,f2,f3,axles,euro,co2_class').eq('company_id',company.id).is('archived_at',null).order('created_at').limit(101),
