...
export default async function Account({searchParams}:{searchParams:Promise<{notice?:string}>}){
 ...
 db.from('vehicles').select('id,plate,registration_country,label,archived_at,technical').eq('user_id',user.id).order('created_at',{ascending:false})]);
 ...
 {rows.map(v=><article className="saved-vehicle" key={v.id}><div><span className="saved-plate">{v.registration_country} · {v.plate}</span><p>{v.label||'Vehiculul meu'}{v.archived_at?' · Arhivat':''}</p></div><details><summary>Editează datele</summary><VehicleForm vehicle={v}/></details>{!v.archived_at&&<Link className="secondary" href={'/cumpara?vehicle='+v.id}>Pregătește vinietele</Link>}<Link className="secondary" href={'/cont/calatorii?vehicle='+v.id}>Planifică o cursă</Link><form action={archiveVehicle}><input type="hidden" name="id" value={v.id}/><input type="hidden" name="archive" value={v.archived_at?'no':'yes'}/><button className="secondary" aria-label={(v.archived_at?'Restaurează ':'Arhivează ')+v.plate}>{v.archived_at?'Restaurează':'Arhivează'}</button></form></article>)}
