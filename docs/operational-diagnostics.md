...
Activarea cere simultan build cu `CONTEXT=deploy-preview`, `POSTHOG_DIAGNOSTICS_ENABLED=true` și tokenul de proiect `POSTHOG_PROJECT_TOKEN`. Destinația publică este fixată în codul serverului la proiectul `279270`, host EU `https://eu.i.posthog.com`; nu necesită variabile secrete pentru ID/host. Tokenul și activarea sunt configurate numai în contextul Netlify Deploy Previews; tokenul nu este expus prin NEXT_PUBLIC sau next.config. Gate-ul de build este false în producție, branch deploy și local, inclusiv dacă variabilele runtime există. Dezactivare rapidă: POSTHOG_DIAGNOSTICS_ENABLED=false în preview și redeploy.
